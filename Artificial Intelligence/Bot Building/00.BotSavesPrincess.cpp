 ...
 *
 * Scoring
 * Your score is calculated as follows : (NxN - number of moves made to rescue the princess)/10, where N is the size
 * of the grid (3x3 in the sample testcase).
 */
...
int main(void) {
  ...
  // Print Moves
  for (int i = 0; i < abs(princess.x - robot.x); i++) {
    cout << horizontal << endl;
  }

  for (int j = 0; j < abs(princess.y - robot.y); j++) {
    cout << vertical << endl;
  }
