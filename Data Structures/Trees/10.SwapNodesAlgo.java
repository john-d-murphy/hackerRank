...
It contains a root node and two subtrees, left subtree and right subtree. These subtrees are also binary trees.
...
Then N lines follow.
Here each of ith line (1 <= i <= N) contains two integers, a b, where a is the index of left child, and b is the index
of the right child of the ith node.
-1 is used to represent null node.
Next line contains an integer, T.
Then again T lines follows.
Each of these line contains an integer K.

Output Format
For each K, perform the swap operation as mentioned above and print the inorder traversal of the current state of tree.
...
Either a = -1 or 2 <= a <= N
Either b = -1 or 2 <= b <= N
Index of (non-null) child will always be greater than that of its parent.
...
Test Case #02: Here we perform swap operations at the nodes whose depth is either 2 and 4 and
then at nodes whose depth is 4.
...
This question is painful. Skipping.
